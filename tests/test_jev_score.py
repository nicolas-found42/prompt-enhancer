        ...
        gateway.decide(
            {
                "type": "score",
                "query": "How well?",
                "levels": list(answer["legend"].values()),
            }
        )
    )
...
def test_score_level_tie_uses_lower_level() -> None:
    decision = parse_decision(
        {"type": "score", "score": 1.5, "probabilities": {"0": 0, "1": 0.5, "2": 0.5}}
    )
...
def test_grading_mixed_tests_uses_correct_answers_and_one_noul_request_each() -> None:
    ...
    def decide(request, **_kwargs):
        ...
        return {
            "type": "noul",
            "probability_true": 0.9 if key == "grade_0_0_first" else 0.1,
        }
    ...
    grades, evidence = grade_panel_with_jev(
        panel, tests, ScriptedGateway(decision=decide), judge_model="jev", run_id="run"
    )
...
def test_unusable_noul_answer_cannot_pass_expected_no() -> None:
    ...
    gateway = ScriptedGateway(
        decision=lambda request, **_kwargs: {
            "type": "noul",
            "probability_true": "invalid",
        }
    )

    grades, _ = grade_panel_with_jev(
        panel, tests, gateway, judge_model="jev", run_id="run"
    )
