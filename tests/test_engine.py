...
def test_clear_prompt_with_success_tests_is_never_rewritten() -> None:
    ...
    result = PromptOptimizer(store=RunStore(":memory:"), gateway=ScriptedGateway(chat=chat, decision=decide)).optimize(prompt)
...
def test_update_assumption_replaces_recorded_value_and_refreshes_report() -> None:
    ...
    assert store.get_run(original["run_id"])["result"]["final_prompt"] == updated["final_prompt"]


def test_update_assumption_rejects_negative_jev_meaning_check() -> None:
    optimizer = PromptOptimizer(store=RunStore(":memory:"), gateway=_clarification_gateway(0.0))
    original = optimizer.optimize("Write a concise report.")
    completed = optimizer.resume(original["run_id"], {"goal": "summarize"})
    ...
    assert optimizer.store.get_run(run_id)["result"]["final_prompt"] == completed["final_prompt"]
...
def test_update_assumption_requires_recorded_assumption_and_jev_answer() -> None:
    ...
    try:
        optimizer.update_assumption(result["run_id"], {"key": "audience", "value": "novices"})
    except ValueError as exc:
        assert "assumption" in str(exc)
    else:
        raise AssertionError("unrecorded assumption should not be editable")

    optimizer = PromptOptimizer(store=RunStore(":memory:"), gateway=_clarification_gateway(None))
    pending = optimizer.optimize("Write a concise report.")
    completed = optimizer.resume(pending["run_id"], {"goal": "summarize"})
    try:
        optimizer.update_assumption(completed["run_id"], {"key": "goal", "value": "analyze"})
...
def _clarification_gateway(meaning_score: float | None, initial_cost: float = 0.0) -> ScriptedGateway:
    ...
    def decide(request, **_kwargs):
        ...
        if key == "gap:goal" and initial_cost:
            gateway.usage.record(role="judge", provider="openrouter", model="typesafe/jev-1.13", cost=initial_cost)
        if key == "task_type":
            return {"type": "choice", "choice": "general", "probabilities": {"general": 1.0}, "confidence": 1.0}
        if key == "infer:goal":
...
def test_engine_rejects_unfaithful_candidate_even_when_weak_models_prefer_it() -> None:
    ...
    result = optimizer.optimize("Original request", {"tier": "fast", "clarification_allowed": False})

    assert result["final_prompt"] == "Safe rewrite"
    assert result["report"]["selection_evidence"]["rejection_reasons"]["candidate-1-add_missing_context"]
...
def test_provider_failure_preserves_original_prompt_and_run_record() -> None:
    ...
    def decide(request, **_kwargs):
        if request.get("type") == "choice":
            choice = "general" if request.get("key") == "task_type" else "none"
            return {"type": "choice", "choice": choice, "probabilities": {choice: 1.0}, "confidence": 1.0}
        probability = 0.99 if str(request.get("key", "")).startswith("gap:goal") else 1.0
        return {"type": "noul", "probability_true": probability, "confidence": 1.0}

    store = RunStore(":memory:")
    optimizer = PromptOptimizer(store=store, gateway=ScriptedGateway(chat=chat, decision=decide))
    result = optimizer.optimize("Explain recursion.", {"tier": "fast", "clarification_allowed": False})
...
def test_resume_after_restart_retains_cost_spent_before_clarification(tmp_path) -> None:
    database = tmp_path / "runs.sqlite3"
    first = PromptOptimizer(store=RunStore(database), gateway=_clarification_gateway(0.95, initial_cost=0.02))
    pending = first.optimize("Write a concise report.")
    assert pending["status"] == "needs_input"
    assert pending["cost"]["total"] == 0.02
    first.store.close()

    resumed = PromptOptimizer(store=RunStore(database), gateway=_clarification_gateway(0.95)).resume(
        pending["run_id"], {"goal": "summarize"}
    )
...
def test_unknown_high_impact_gap_offers_writer_choices_and_preserves_diagnosis() -> None:
    def chat(_model, _messages, **_kwargs):
    ...
    def decide(request, **_kwargs):
        ...
        if request.get("key") == "infer:goal":
            return {"type": "choice", "choice": "unknown", "probabilities": {"summarize": 0.6, "analyze": 0.2, "unknown": 0.2}, "confidence": 0.6}
        if request.get("type") == "choice":
            return {"type": "choice", "choice": "none", "probabilities": {"none": 1.0}, "confidence": 1.0}
    ...
    assert [option["value"] for option in question["options"]] == ["summarize", "analyze", "other"]
