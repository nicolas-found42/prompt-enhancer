...
def test_precision_floor_selects_the_lowest_cutoff_that_keeps_half_the_flags_right(tmp_path: Path) -> None:
    def bucket(group: str) -> int:
        return int(hashlib.sha256(group.encode()).hexdigest()[:8], 16) % 5

    train = next(f"participant-{index}" for index in range(100) if bucket(f"participant-{index}") != 0)
    holdout = next(f"participant-{index}" for index in range(100) if bucket(f"participant-{index}") == 0)
    # Training: true gaps at 0.70 and 0.84, false flags at 0.75, 0.62 and 0.61.
    scores = [
        (train, 0.70, True), (train, 0.84, True), (train, 0.75, False), (train, 0.62, False),
        (train, 0.61, False), (holdout, 0.9, True),
    ]
    dataset = {
        "name": "reviewed", "metadata": {"reviewer_kind": "user_delegated_model"},
        "cases": [
            {"id": f"case-{i}", "task_stratum": "chat", "source_group": group,
             "expected_gaps": ["context"] if gap else [], "label_provenance": "user_delegated_model"}
            for i, (group, _, gap) in enumerate(scores)
        ],
    }
    decisions = {"dataset_name": "reviewed", "rows": [
        {"case_id": f"case-{i}", "source_group": group, "probabilities": {"goal": 0.2, "context": score}}
    ...
    report = calibrate(dataset_path, decisions_path, selection="precision_floor", min_precision=0.5)
