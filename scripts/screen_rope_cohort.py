...
_PLACEHOLDER = re.compile(r"\b(?:XXX|TBD|placeholder|fill in)\b|\[[^\]]+\]|<[^>]+>|\{[^{}]+\}", re.IGNORECASE)
...
def screen(dataset: dict[str, Any]) -> dict[str, Any]:
    ...
    retained = [case for case in cases if isinstance(case, dict) and not _PLACEHOLDER.search(str(case.get("prompt", "")))]
    ...
    metadata["task_counts"] = dict(Counter(str(case["task_stratum"]) for case in retained))
    return {**dataset, "name": str(dataset["name"]) + "-screened", "metadata": metadata, "cases": retained}
