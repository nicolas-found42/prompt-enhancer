...
def calibrate(
    review_path: Path, evidence_path: Path, *, minimum: int = 80
) -> dict[str, Any]:
    ...
    for row in review:
        ...
        if (
            hashlib.sha256(content).hexdigest() != item["content_digest"]
            or row["source_case_id"] != item["source_case_id"]
            or row["writer"] != item["writer"]
        ):
        ...
        (holdout if grouped_holdout else train).append(
            (float(item["probability"]), label == "yes")
        )
        provenances.add(reviewer_kind)
    if len(train) + len(holdout) < minimum:
        raise ValueError(
            f"only {len(train) + len(holdout)} yes/no judgments; require {minimum}"
        )
    ...
    selected = max(
        thresholds, key=lambda value: (binary_metrics(train, value)["f0_5"], value)
    )
    return {
        "question": evidence["question"],
        "label_provenance": next(iter(provenances)),
        "reviewed": len(train) + len(holdout),
        "uncertain": uncertain,
        "train_count": len(train),
        "holdout_count": len(holdout),
        "selection": "Maximum training F0.5 on 0.50–0.95 grid; source conversations grouped; higher cutoff breaks ties",
        "previous_threshold": PREVIOUS_THRESHOLD,
        "current_threshold": DEFAULT_FAITHFULNESS_THRESHOLD,
        "selected_threshold": selected,
        "previous_train": binary_metrics(train, PREVIOUS_THRESHOLD),
        "previous_holdout": binary_metrics(holdout, PREVIOUS_THRESHOLD),
        "current_train": binary_metrics(train, DEFAULT_FAITHFULNESS_THRESHOLD),
        "selected_train": binary_metrics(train, selected),
        "current_holdout": binary_metrics(holdout, DEFAULT_FAITHFULNESS_THRESHOLD),
        "selected_holdout": binary_metrics(holdout, selected),
    }
...
def main() -> None:
    ...
    rendered = (
        json.dumps(calibrate(args.review, args.evidence), indent=2, sort_keys=True)
        + "\n"
    )
    if args.output:
