...
def review(paths: list[Path], output: Path, raw_path: Path, *, batch_size: int) -> None:
    ...
    for start in range(0, len(cases), batch_size):
        ...
        response = _json_reply(
            gateway, instruction=instruction, cases=prepared, max_tokens=4500
        )
        decisions = response["decisions"]
        if len(decisions) != len(current) or {item.get("id") for item in decisions} != {
            case["id"] for case in current
        }:
            raise ValueError(f"wrong case IDs in batch {start}")
        raw["batches"].append(
            {"case_ids": [case["id"] for case in current], "response": response}
        )
            ...
            saved["cases"].append(
                {
                    ...
                    "label_notes": str(decision.get("notes", "")),
        ...
        print(
            f"Public gap review {len(done)}/{len(cases)}: {dict(Counter(c['task_stratum'] for c in saved['cases']))}",
            flush=True,
        )
