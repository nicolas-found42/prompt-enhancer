    ...
    with path.open(encoding="utf-8", errors="replace") as source:
        for line_number, line in enumerate(source, 1):
            try:
                ...
                continue
            if event.get("type") not in {"user", "assistant"} or event.get("isMeta") or event.get("isSidechain"):
                continue
...
def repair(batch: dict[str, Any], session_root: Path) -> tuple[dict[str, Any], list[str]]:
    cases = batch["cases"]
    ...
    for case in cases:
        ...
        if not any(role == "user" and prompt in _normalized(text) for _, role, text in messages):
    ...
    for path in session_root.glob("**/*.jsonl"):
        ...
        if not messages:
            continue
        first_user = next(((index, line, text) for index, (line, role, text) in enumerate(messages) if role == "user"), None)
    ...
    if len(candidates) < len(invalid):
        raise ValueError(f"need {len(invalid)} replacement sessions; found {len(candidates)}")
    replacement_by_id = {case_id: {"id": case_id, **candidate} for case_id, (_, candidate) in zip(invalid, candidates, strict=False)}
    revised = {**batch, "cases": [replacement_by_id.get(case["id"], case) for case in cases]}
    revised["notes"] = str(batch.get("notes", "")) + " Claude metadata and sidechain entries were replaced with verified main-session first user turns."
    if len({(case["source"], case["session"]) for case in revised["cases"]}) != len(revised["cases"]):
...
def main() -> None:
    ...
    parser.add_argument("--session-root", type=Path, default=Path.home() / ".claude" / "projects")
    ...
    print(f"Replaced {len(invalid)} non-main Claude entries; retained {len(revised['cases'])} cases")
