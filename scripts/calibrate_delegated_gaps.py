...
def measure(dataset_path: Path, decisions_path: Path) -> None:
    ...
    cases = dataset["cases"]
    if dataset.get("metadata", {}).get(
        "reviewer_kind"
    ) != "user_delegated_model" or any(
    ...
    for case in cases:
        ...
        questions = [
            {
                "model": JEV_MODEL,
                "query": default_gap_question(gap),
        ...
        saved["rows"].append(
            {
                ...
                "probabilities": {
                    gap: _probability(answer)
                    for gap, answer in zip(
                        TASK_GAPS[case["task_stratum"]], answers, strict=True
                    )
                },
            }
        )
...
def _select(
    train: list[tuple[float, bool]], selection: str, min_precision: float
) -> float | None:
    if selection == "precision_floor":
        ...
        eligible = [
            cutoff
            for cutoff in grid
            if (metrics := binary_metrics(train, cutoff))["tp"] >= 1
...
def calibrate(
    ...
    *,
    ...
    if dataset.get("metadata", {}).get(
        ...
        case.get("label_provenance") != "user_delegated_model"
    ...
    by_gap: dict[str, dict[str, list[tuple[float, bool]]]] = defaultdict(
    ...
    for case_id, case in cases.items():
        ...
        for gap in TASK_GAPS[case["task_stratum"]]:
            by_gap[gap][split].append(
                (row["probabilities"][gap], gap in case["expected_gaps"])
            )
    result = {
        "dataset_name": dataset["name"],
        "label_provenance": "user_delegated_model",
        "decision_model": JEV_MODEL,
        "split": "SHA-256 source_group modulo 5; related participant prompts stay together",
    ...
    for gap in GAPS:
        ...
        result["questions"][gap] = {
            "question": default_gap_question(gap),
            "train_count": len(train),
            "holdout_count": len(holdout),
            "train_positive": sum(label for _, label in train),
            "holdout_positive": sum(label for _, label in holdout),
