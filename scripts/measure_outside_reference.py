            ...
            [
                {
                    ...
                    "type": "noul",
                    "key": f"gap:outside_reference:{case['id']}",
                }
                for case in chunk
            ]
        )
            ...
            saved["rows"].append(
                {
                    ...
                    "probability": float(
                        answer.get("noul", answer.get("probability_true"))
                    ),
                }
            )
        save_json(decisions_path, saved)
        print(
            f"outside_reference decisions {len(saved['rows'])}/{len(dataset['cases'])}",
            flush=True,
        )
...
def summarize(dataset: dict[str, Any], decisions_path: Path) -> dict[str, Any]:
    ...
    no_gap = [
        row["probability"] for row in rows if not cases[row["case_id"]]["expected_gaps"]
    ]
    context = [
        row["probability"]
        for row in rows
        if "context" in cases[row["case_id"]]["expected_gaps"]
    ]
    return {
        "dataset_name": dataset["name"],
        "question": default_gap_question("outside_reference"),
        "current_threshold": DEFAULT_RUBRIC.gap_threshold_for("outside_reference"),
        "no_gap_count": len(no_gap),
        "context_gap_count": len(context),
        "cutoffs": {
            str(cutoff): {
                "no_gap_flags": sum(value >= cutoff for value in no_gap),
