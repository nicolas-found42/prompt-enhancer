...
def collect(
    dataset_path: Path, replays: dict[str, Path], *, per_writer: int = 50
    ...
    evidence: dict[str, Any] = {
        ...
        "replay_digests": {},
        "question": "Is this proposed success test faithful to the user's request, and does it test success rather than an invented requirement?",
        "rows": {},
    }
    for writer, replay_path in replays.items():
                ...
                candidates.append(
                    (
                        row_id,
                        {
                            ...
                            "writer": writer,
                            "prompt": case.prompt,
                            "proposed_test": json.dumps(proposed, ensure_ascii=False),
                            "human_faithful": "",
                            "human_notes": "",
                            "human_reviewer": "",
                            "reviewed_at": "",
        ...
        selected = sorted(
            candidates, key=lambda item: hashlib.sha256(item[0].encode()).hexdigest()
        )[:per_writer]
        if len(selected) < per_writer:
            raise ValueError(
                f"only {len(selected)} faithfulness proposals for {writer}"
            )
...
def main() -> None:
    ...
    rows, evidence = collect(
        args.dataset,
        {
            "space-bunny-free": args.bunny_replay,
            "deepseek-v4.1-flash": args.deepseek_replay,
        },
    )
