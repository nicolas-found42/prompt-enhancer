  ...
  return (
    ...
    >
      ...
      <div className="model-picker-intro">
        <p>
          Change models for this run, or save these choices as your defaults.
          The judge model is fixed.
        </p>
        <button
          className="secondary"
          type="button"
      ...
      <p id="writer-hint" className="role-hint">
        Writes the improved versions and the checks for a good answer.
      </p>
      ...
      <p id="strong-hint" className="role-hint">
        A capable model. An improved version must not do worse on it than your
        prompt.
      </p>
      <fieldset>
        <legend>Weak panel ({selection.weak.length} selected)</legend>
        <p className="role-hint">
          Cheaper test models. Each improved version is tried on these and must
          help them.
        </p>
        {chosenWeak.map((model) => (
          <label key={model.id} className="chosen">
            <input
              type="checkbox"
              checked
              onChange={(event) => toggleWeak(model.id, event.target.checked)}
            />
            {optionLabel(model, providers)}
          </label>
        ))}
