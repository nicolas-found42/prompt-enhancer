...
export default function RunProgress({ job, estimate, onCancel }: Props) {
  ...
  const round = job.round.round && job.round.max_rounds && job.round.max_rounds > 1
    ? ` · round ${job.round.round} of ${job.round.max_rounds}`
    : "";

  return (
    <section className="result progress" aria-live="polite" aria-labelledby="progress-heading">
      <div className="result-heading">
        ...
        <button className="secondary" type="button" onClick={onCancel} disabled={job.cancel_requested}>
          {job.cancel_requested ? "Cancelling…" : "Cancel"}
        </button>
      </div>
      <ol className="stages">
        {stageOrder.map((stage, index) => {
          const state = job.state === "queued" ? "pending" : index < current ? "done" : index === current ? "current" : "pending";
          return (
            <li key={stage} className={`stage stage-${state}`} aria-current={state === "current" ? "step" : undefined}>
              {stageLabels[stage]}
            </li>
          );
        })}
      ...
      <p className="progress-note">
        You can leave this page open or come back later. The run keeps going and will show up in History.
      </p>
    </section>
  );
}
