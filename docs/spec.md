...
- **Grader.** Grades every (candidate × model × sample) output against the tests in batched Jev requests: many items per request, each question naming its item. Computes each candidate's pass rate per model, worst-model pass rate, mean pass rate and spread between samples. Self-checks: important questions are also asked in reversed form (the two answers should add up to about 1), and option order is shuffled for `choice` comparisons, because the two independent audits disagree on whether Jev has a position bias. Questions whose answers disagree get less weight.
