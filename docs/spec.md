...
- **Jev client.** Builds Decisions API requests and parses answers into typed results (`noul` probability; `choice` pick, per-option probabilities and confidence; `score` level, per-level probabilities and confidence). The user's prompt and all model outputs always go in `state`, never in `instructions`, so text being judged can't steer the question.
...
- **Grader.** Grades every (candidate × model × sample) output against the tests in batched Jev requests: many items per request, each question naming its item. Computes each candidate's pass rate per model, worst-model pass rate, mean pass rate and spread between samples. Each Noul success test uses one direct question; an expected "no" uses the complement of that answer. The reversed-form self-check was dropped because [Jev 1.13 does not guarantee that separate questions and their negations sum to 1](https://docs.typesafe.ai/model-jaggedness/jev-1.13). Option order is reversed for `choice` comparisons because the two independent audits disagree on whether Jev has a position bias. Choice answers whose probabilities disagree get less weight.
